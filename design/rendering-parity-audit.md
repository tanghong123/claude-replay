...
| 3.17 | `request_user_input` card | was: the wire carried `head.interaction` and the page ignored it, showing a generic tool fold. Now the card, inside the fold body | waiting/resolved card with answers, unchanged | `shared/interaction.js` (#121) holds the two states, their words ("Waiting for user input" / "User input received", where to answer, what was answered) and the markup; each page passes its own class names, and neither holds the words | GUARDED (v1.193.0; every question and option, #255; every question drawn alike, with the reader's typed words and notes, #280; a call that came back without an answer settles as No answer and says why — timed out, declined, failed — #281; an option's preview, the asker's drawing, on demand but for the chosen one's, #282) | ✓ both: `scenario_a_request_for_input_is_a_card`, `both_shells_show_every_question_and_every_option_that_was_offered`, `both_shells_draw_every_question_alike_and_every_answer_given`, `both_shells_say_why_a_question_went_unanswered`, `both_shells_show_the_drawings_an_asker_gave_its_options` |
...
|---|---|---|---|
| `shared/parts.js` (landed, #108, #122) | the row caps: split, label, `num`/`diff` row markup with a class map, the expansion memory keyed by record id + ordinal, the `pre` line rule (a trailing newline ends the last line); the result body — the ⎿ gutter and the output beside it (#122) | classic `capped`/`numberedRows`/`diffRows` run on it, keeping their DOM and ordinal stamping; app `partsHtml`/`codeRows` render from it at render time with in-place expansion | 3.1 done; 3.2 (gutters), `md`/`note`/`blocks` markup and wrap classes remain per page for now |
...
| `shared/tool-head.js` (landed, #117) | name/target/chips → state (failed/completed/none), the status word, exit, duration and line count, the display-name rule; a head carries no liveness | classic fold header pieces run on it; app `viewRecord`'s chip regexes are gone | 3.4 done |
| `shared/search.js` (landed, #111, #118) | the haystack (a record's text, nested included), the scope classes, the whole-word rule and occurrence counting — both pages run it; still per page: per-scope counts, the typed prefix, hit order, mark placement (#101, #104, #118) | classic `ownTextParts`/`directMask`/`countOcc`/`wholeAt` run on it; app `plainText` does | 5.3, 5.4 done; #101, #104, #118 |
| `shared/filter.js` (landed, #118) | the chain: a record matches, or something under it does, and every fold on the way is on the chain (`chainWalk`) — what MATCHES and what a match opens stay each page's own | classic `recMatch` + `openFilterChain`; app `filterChain` | 5.8 (hide vs landmark, snapshot restore: each page's own) |
...
| `shared/interaction.js` (landed, #121) | the request-for-input card: its two states, their words and its markup | app `rendererBody`'s inline card; the classic page had none | 3.17 done |
| `shared/task-card.js` (landed, #125) | how a task READS: the status glyphs, the chips (state, owner, checks, blockers), the `created · claimed · completed` line, the row's second line, and the card's sections — description, acceptance, outcome, worklog | classic `renderTasks`' deps+desc; app `taskDetails`' facts list | the owner's report, 2026-09-04 |
...
| `shared/virtual-window.js` (landed, #107) | the window's ARITHMETIC — prefix sums, index-at-offset (clamped or not), the two window ranges, the pads, the measure threshold, the anchor correction, first-visible, the follow rule — numbers in, numbers out, so the node contract can run it | both pages run it: app `viewport.js` and classic `P`/`idxAt`/`updatePads`/`captureAnchor`/`restoreAnchor`/the scroll handler. the ENGINE too since step 4a (`VirtualWindow` + a frame adapter): the window, the pads, the anchor, both observers, the measure schedule, the follow state, the thumb, the converge, the remember debounce — the app shell's viewport is its consumer. Still each page's own: what an item IS, how one renders, and what a reveal opens | #107 steps 1–4a (v1.195–v1.198); the classic page driving the class through a document frame is what remains |
