...
| `shared/virtual-window.js` (step 1 landed, #107) | the window's ARITHMETIC — prefix sums, index-at-offset (clamped or not), the two window ranges, the pads, the measure threshold, the anchor correction, first-visible, the follow rule — numbers in, numbers out, so the node contract can run it | app `viewport.js` runs it; the classic page adopts it at step 2 | #107 |
