...
| 1.4 | **Raw text of a user turn** | `{}` shows `src` — exactly as typed, whitespace intact; per turn (`.rawbtn`) and global (`#btn-raw`, persisted); wrap applies | `{}` shows `JSON.stringify(record)` — the record, not the text; per turn only; nothing persisted | **KEEP** (port) | none → add: a prompt with leading indentation reads verbatim under raw on both |
...
| # | Feature | Classic page | App shell | Verdict | Scenario |
|---|---|---|---|---|---|
| 3.1 | **Output caps** (`⋯ N more lines · to line M`) on `pre`/`num`/`diff` parts | first 12 (pre, diff) / 10 (num) rows, the rest behind a button; small expansions remembered per record | the server's `cap` is ignored: every row is in the DOM, a 360 px scroll box clamps it visually | **KEEP** (port) — a 5,000-line Bash result is 5,000 rows today, and reading through a scroll-box inside a scroller is the worst of both | none → add: a 200-line result shows the cap and its expander on both |
...
|---|---|---|---|
| `shared/parts.js` | body parts → markup: `md`/`think`/`pre`/`raw`/`num`/`diff`/`note`/`blocks`; row caps and the `⋯ N more lines · to line M` expander; expansion memory keyed by record id + ordinal; unselectable gutters; wrap classes | classic `renderPart`/`numberedRows`/`diffRows`/`capped`; app `partsHtml`/`codeRows` | 3.1, 3.2 |
