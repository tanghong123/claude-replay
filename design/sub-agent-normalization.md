...
1. **Keying + when the id is known.** For Claude, the spawn `tool_use` does **not** carry the
   agent id — it arrives on the spawn's **`tool_result`** (`toolUseResult.agentId`, the `user`
   message closing the `Task`/`Agent` call), where the fold back-patches it onto the
   `SubAgent` (join by `tool_use_id`). For Codex, the collaboration spawn result identifies
   an `agent_path`, while the real child session id arrives in the child rollout's
   `session_meta`; an operation-scoped adapter resolver correlates parent session id +
   `agent_path` to that rollout. Until either format supplies a stable id, key the spawn by
   `tool_use_id` (or a synthetic key), then re-key it when the adapter resolves the id.
   Blocks reference whatever key the map uses. (`apply_completions_and_suppress` runs later
   and only back-patches terminal state when the id is already known.)
...
4. **Discovery of the paths.** `transcript` comes from the adapter's relationship resolver:
   Claude derives it from the flat `subagents/` layout; Codex resolves it from the anchored
   parent/child rollout graph (`parent_thread_id` + `agent_path`). `output_file` comes from
   the spawn result when that format records one.
