...
fn session_from_path(path: &Path) -> Option<CodexSession> {
    ...
    for line in BufReader::new(file).lines().map_while(Result::ok).take(100) {
        ...
        return Some(CodexSession {
            ...
            cwd,
            mtime,
            interactive: originator == "codex-tui",
        });
    }
    None
}
...
pub(crate) fn latest_interactive_for_cwd_in(root: &Path, cwd: &Path) -> Result<CodexSession> {
    let wanted = normalized(cwd);
    sessions_in(root)
        .into_iter()
        .find(|session| session.interactive && normalized(&session.cwd) == wanted)
        .ok_or_else(|| {
            anyhow!(
                "no interactive Codex session found for {} under {}",
                cwd.display(),
                root.display()
            )
        })
...
mod tests {
    ...
    #[test]
    fn selects_newest_interactive_session_for_exact_cwd() {
        let fixture = Fixture::new();
        let repo = fixture.root.join("repo");
        let other = fixture.root.join("other");
        fixture.rollout("2026/07/17", "old", &repo, "codex-tui");
        std::thread::sleep(Duration::from_millis(10));
        fixture.rollout("2026/07/18", "background", &repo, "codex-exec");
        std::thread::sleep(Duration::from_millis(10));
        fixture.rollout("2026/07/18", "other", &other, "codex-tui");
        std::thread::sleep(Duration::from_millis(10));
        fixture.rollout("2026/07/18", "wanted", &repo, "codex-tui");

        let selected = latest_interactive_for_cwd_in(&fixture.sessions, &repo).unwrap();
