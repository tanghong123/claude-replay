...
impl<S: BlockStore> Body<S> {
    ...
    fn committed_tail(&self, from: usize) -> Vec<Block> {
        match self {
            ...
            Body::Hibernated(h) => {
                let mut blocks: Vec<_> = h.committed[from.min(h.committed.len())..]
                    .iter()
...
impl<S: BlockStore> SharedSession<S> {
    ...
    pub fn with_store(agent: Agent, path: &Path, store: S) -> Self {
        Self::from_transcript_with_store(Transcript::open(agent, path), store)
    }

    /// Open from a source handle so related sessions retain their operation-scoped resolver.
    pub fn from_transcript_with_store(source: Transcript, store: S) -> Self {
        Self {
            inner: Mutex::new(Inner {
                body: Body::Live(Box::new(source.follow_with_store(store))),
                epoch: 1,
                provisional_gen: 0,
                n_provisional: 0,
...
mod tests {
    ...
    #[test]
    fn codex_pull_and_restore_expose_stable_child_rollout_ids() {
        use crate::engine::tier_b::TierBStore;
        static CODEX_N: AtomicUsize = AtomicUsize::new(0);
        let root = std::env::temp_dir().join(format!(
            "cr-shared-codex-{}-{}",
            std::process::id(),
            CODEX_N.fetch_add(1, Ordering::Relaxed)
        ));
        ...
        let backing = root.join("parent.blocks");
        let sidecar = root.join("parent.state");
        std::fs::write(
            &parent,
            concat!(
                r#"{"type":"session_meta","payload":{"id":"parent","cwd":"/repo","source":"cli"}}"#,
                "\n",
                r#"{"type":"response_item","payload":{"type":"message","role":"user","content":[{"type":"input_text","text":"start"}]}}"#,
                "\n",
                ...
                "\n",
                r#"{"type":"response_item","payload":{"type":"message","role":"assistant","content":[{"type":"output_text","text":"waiting"}]}}"#,
                "\n",
                r#"{"type":"response_item","payload":{"type":"message","role":"user","content":[{"type":"input_text","text":"next"}]}}"#,
                "\n",
            ),
        )
        .unwrap();
        std::fs::write(
            &child,
            concat!(
                r#"{"type":"session_meta","payload":{"id":"child-uuid","cwd":"/repo","source":{"subagent":{"thread_spawn":{"parent_thread_id":"parent","agent_path":"/root/review","agent_nickname":"Reviewer"}}}}}"#,
                "\n"
            ),
        )
        .unwrap();

        let source = Transcript::open(Agent::Codex, &parent);
        let live =
            SharedSession::from_transcript_with_store(source, TierBStore::file(&backing).unwrap());
        live.advance().unwrap();
        let delta = live.pull_delta(live.epoch(), 0);
        assert_eq!(delta.meta.children.len(), 1);
        assert_eq!(delta.meta.children[0].id, "child-uuid");
        assert_eq!(delta.meta.children[0].agent_type, "Reviewer");
        assert!(delta.committed_delta.iter().any(
            |block| matches!(block, Block::SubAgent(agent) if agent.agent_id == "child-uuid")
        ));

        live.hibernate(&sidecar).unwrap();
        drop(live);
        let restored =
            SharedSession::restore(&parent, &sidecar, &backing).expect("unchanged source restores");
        let full = restored.pull_delta(0, 0);
        assert_eq!(full.meta.children[0].id, "child-uuid");
        assert!(full.committed_delta.iter().any(
            |block| matches!(block, Block::SubAgent(agent) if agent.agent_id == "child-uuid")
        ));
