...
// `latest_tree_activity` / `inflight_tool_in_tail` moved DOWN to claude-replay-core
// (#98 §10): they are transcript-shaped, agent-neutral file reads, and a separate monitor
// crate must link them without dragging in clap/ratatui/the whole viewer. Re-exported here
// so every existing `jdi::` caller keeps its path.
pub use claude_replay_core::liveness::{inflight_tool_in_tail, latest_tree_activity};
