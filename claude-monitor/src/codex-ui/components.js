...
function renderRenderer(view, index, state, inherited) {
  ...
  // The target is ONE string for both heads (#231). It used to be interpolated into the
  // interactive head and hard-coded EMPTY into the other, so any record whose body renders to
  // nothing lost the very thing it was about: a Read whose result is an image becomes a separate
 ...
 *  row per member — a dot that pulses while it runs, the member's name opening its session, and
 *  its id. The members ride the META, not the record, so this is rebuilt whenever either
 *  changes; a run the meta does not know yet renders nothing. */
