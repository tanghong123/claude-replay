...
// and the scrollbar thumb owns the position while it is held.
{
  ...
  assert.match(src, /restoreModelAnchor\(held, immediate = false\) \{\n    const want = this\.documentTopOf\(held\.index\) \+ held\.offset;/, "#191: the model restore is the record the offset named plus how far into it — the sums' own position, not a replay");
...
// was deferred and then dropped. Two rules, both pure enough to check here.
{
  ...
  assert.match(engine, /if \(anchor\) this\.restoreDomAnchor\(anchor, true\);/, "the late apply holds the anchor across the shift, immediately");
