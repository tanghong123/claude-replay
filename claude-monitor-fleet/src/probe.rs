...
/// under `$XDG_CACHE_HOME` or `~/.cache`. That last glob is what finds a second monitor without
/// being told its name — `agent-monitor-next`, `-staging`, whatever the user called it — and it is
/// a heuristic about THIS TOOL's own directory naming, not about the user's machines. A root
/// somewhere else entirely is reachable with `--cache-root`.
///
